#############################################################
######### Simulation function for univariate outcome ########
#############################################################
### Input
...
gmm_simulation <- function(data.sim,pars_true,X,Z,plot=FALSE,
                            P=1,n_class=2,nloop=110,burnin=10,thin=1)
{
  ...
  for(i in 1:N){
    ...
    K_true = c(K_true,K_i)
    data.sim$K[data.sim$id == i] = K_i
    T_i = cbind(1,data.sim$age[data.sim$id == i])
    A_true[i,] = rmvnorm(1,alpha_true[,K_i],Sigma_a_true[,,K_i])
    data.sim$y[data.sim$id == i] = X[[i]]%*%beta_true + T_i%*%(cbind(A_true[i,])) + rnorm(TT,0,s_sq_eps_true^.5)
  }
  pars_true[[4]] = A_true
  pars_true[[7]] = K_true
  ...
  p = .75
  for(i in 1:N){
    data.sim$keep[data.sim$id == i] = rbinom(sum(data.sim$id == i),1,p)
    X[[i]] = rbind(X[[i]][which(data.sim$keep[data.sim$id == i] == 1),])
  }
  ...
  for(i in 1:N){
    Y_sparse[[i]]=list()
    Y_sparse[[i]][[1]] = data.sim$y[data.sim$id == i]
    time_sparse[[i]]=list()
    time_sparse[[i]][[1]] = data.sim$age[data.sim$id == i]
  }
  
  gmm.fit = gmm(Y_sparse=Y_sparse,time_sparse=time_sparse,
                 X=X,Z=Z,n_class = n_class,
