##########################################################################
### Growth mixture model for multivariate outcome and unequal variance ###
##########################################################################
### Input
...
mgmm=function(Y_sparse,time_sparse,M_i_sparse,X,Z,P=1,n_class=2,nloop=110,burnin=10,thin=1,sim=TRUE, pars = NULL){
  ...
  if(sim==TRUE){ ## specified as true simulated values and no estimation needed if 0.
    ...
    est_A = 1
    if(est_A == 0) A = pars[[4]]
    if(est_A == 1) A = array(0,dim=c(N,2*P))
  ...
  xi = matrix(1,1,2*P) #larger values provide weakly informative priors on variances 
  ...
  for(ii in 1:nloop){
    ...
    if(est_alpha == 1){
      for(k in 1:n_class){
        N_k = sum(K == k)
        Sigma_alpha_k = solve(N_k*solve(Sigma_a[,,k])+diag(rep(1,2*P))/cc)
